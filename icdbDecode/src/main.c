...
FILE* myfopen(char*, char*, int, char*, int);
...
int main(int argc, char** argv)
{
	...
	for (int i = 0; i < argc; ++i)
	{
		...
		{	// Database
			...
			storepath = malloc(storepathLenth);
			memcpy(storepath, argv[i + 1], storepathLenth);
			printf("Using storing path: \t[%s]\n", storepath);
		}
		else if ((strcmp(argv[i], "-c") == 0 || strcmp(argv[i], "-C") == 0) && nontDecompress == 0)
		{
			nontDecompress = 1;
		}
		else if ((strcmp(argv[i], "-q") == 0 || strcmp(argv[i], "-Q") == 0) && quietMode == 0)
		{
			quietMode = 1;
		}
		else if ((strcmp(argv[i], "-l") == 0 || strcmp(argv[i], "-L") == 0) && linkDoubleFiles == 0 && noDoubleFiles == 0)
		{
			linkDoubleFiles = 1;
		}
		else if ((strcmp(argv[i], "-n") == 0 || strcmp(argv[i], "-N") == 0) && noDoubleFiles == 0 && linkDoubleFiles == 0)
		{	
			noDoubleFiles = 1;
		}
	...
	logFile = myfopen("w", storepath, storepathLenth, LOGFILE_NAME, sizeof(LOGFILE_NAME));
	if (logFile == 0)
	{
...
/* Read in, and print database header */
...
/* Read in, and print data file list */
...
/* Read in file payload, unite fragmented files, export files */
int exportFiles(FILE* sourceFile, databaseHeader_Struct* databaseHeader, file_struct** file)
{
	...
	for (unsigned int i = 0; i < databaseHeader->num_files; i++)
	{
		...
		if(DublicateTo == 0 || (linkDoubleFiles == 0 && noDoubleFiles == 0))
		{
			...
			FILE* destFile = myfopen("wb", storepath, storepathLenth, file[i]->filename, file[i]->filename_length);
			if (destFile != 0)
			{
				// Allocate memory
...
char* makePath(char* absolutepath, int absolutepathlength, char* filepath, int filepathlength)
{
	...
	// Buffer for complete filepath
	int completePathLength = filepathlength + absolutepathlength;
	char* completePath = (char*)calloc(completePathLength + 1, sizeof(char));
	returnpath = (char*)calloc(completePathLength + 1, sizeof(char));
	if (completePath != 0 && returnpath != 0)
	{
		// Unite the path
		memcpy(completePath, absolutepath, absolutepathlength);
		memcpy(completePath + absolutepathlength, filepath, filepathlength);
		completePath[completePathLength] = '\0'; // Zero terminate path

		// Create folder structure
		for (int i = 0; i < completePathLength; i++)
		{
			if ((completePath[i] == '\\') || (completePath[i] == '/'))
			{
				#ifdef WIN32 // Building for Windows
					mkdir(returnpath);
					returnpath[i] = DIR_SEPERATOR_WINDOWS; // Replace Slash / Backslash
				#else // Building for Unix
					mkdir(returnpath, 0777);
					returnpath[i] = DIR_SEPERATOR_UNIX; // Replace Slash / Backslash
				#endif
			}
			else
			{
				returnpath[i] = completePath[i];
			}
		}
		free(completePath);
	}
...
/* Assemble & create filepath, open file */
FILE* myfopen(char* option, char* absolutepath, int absolutepathlength, char* filepath, int filepathlength)
{
	FILE* returnpath = 0;
	char* path = makePath(absolutepath, absolutepathlength, filepath, filepathlength);
	if (path != 0)
	{
...
/* Assemble & create filepath, create link to existing file */
...
/* Decode and print GUID to match format in DXD logs */
void printGUID(uint8_t Guid[24])
{
...
/* Decompress and write data to file. Uses Zlib */
